...
public class RatingViewSoccer extends VBox {
    public RatingViewSoccer(Stage stage) {
        setSpacing(15);
        setAlignment(Pos.CENTER);
        setStyle("-fx-background-color: #f4f4f4;");

        // Caricamento immagine sopra il titolo
        InputStream imgStream = getClass().getResourceAsStream("/img/palloneCalcio.jpg");
        ...
        table.setStyle("-fx-background-color: white; -fx-border-color: #2e7d32; -fx-border-width: 2px;");
        table.setColumnResizePolicy(TableView.CONSTRAINED_RESIZE_POLICY);

        // Creazione colonne con numerazione automatics e intestazioni verdi
        ...
        backButton.setStyle("-fx-background-color: #2e7d32; -fx-text-fill: white; -fx-font-size: 14px; -fx-padding: 6 12; -fx-background-radius: 8; -fx-cursor: hand;");
    ...
    private <T> TableColumn<Player, T> createStyledColumn(String title, String property) {
        ...
        column.setStyle("-fx-text-fill: #2e7d32; -fx-font-weight: bold;");
        return column;
    }
        ...
        try (Connection conn = DatabaseManager.getConnection();
             PreparedStatement stmt = conn.prepareStatement(
                     "SELECT ROW_NUMBER() OVER (ORDER BY RATING DESC) AS POSIZIONE, " +
                             ...
                             "FROM PLAYERS WHERE SPORT = 'S' ORDER BY RATING DESC");
